...
GOAL_EVENT = Event(event_type="score", time="1", player="Claudio Lopez")
...
class Revisionismo_test_suite(unittest.TestCase):
    ...
    def test07_side_rule_gives_extra_point_to_home_team_with_more_than_3_goals(self):
        home_events = [GOAL_EVENT, Event(event_type="score", time="2", player="Claudio Lopez"),
                       Event(event_type="score", time="3", player="Claudio Lopez")]
    ...
    def test08_side_rule_gives_extra_point_to_away_team_with_more_than_3_goals(self):
        events = [GOAL_EVENT, Event(event_type="score", time="2", player="Claudio Lopez"),
                  Event(event_type="score", time="3", player="Claudio Lopez")]
    ...
    def test09_side_rule_gives_extra_point_to_team_that_saved_a_penalty(self):
        events = [GOAL_EVENT, Event(event_type="pk_save", time="1", player="")]
        partido = Partido("Racing", "Independiente", events, [])
    ...
    def test10_single_rule_gives_extra_point_to_team_that_saved_a_penalty(self):
        events = [Event(event_type="score", time="90 +1", player="Chris")]
        partido = Partido("Racing", "Independiente", events, [])
        single_rule = SingleRule(name="single_rule", event_type="score", condition="90 +0", points=1)
